...
with open("/etc/ssl/certs/iplantc_combined.crt","r") as cert:
    certfile_text = base64.b64encode(cert.read())
with open("/etc/ssl/private/iplantc.key","r") as keyfile:
    keyfile_text = keyfile.read()
SAML_SETTINGS = {
    "strict": True,
    "debug": True,
    "sp": {
        "entityId": SERVER_URL + "/auth/samlsso/metadata/",
        "assertionConsumerService": {
            "url": SERVER_URL + "/auth/ws02/?acs",
            "binding": "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
        },
        "singleLogoutService": {
            "url": SERVER_URL + "/auth/ws02/?sls",
            "binding": "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect"
        },
        "NameIDFormat": "urn:oasis:names:tc:SAML:2.0:nameid-format:unspecified",
        "x509cert": certfile_text,
        "privateKey": keyfile_text,
    },
    "idp": {
